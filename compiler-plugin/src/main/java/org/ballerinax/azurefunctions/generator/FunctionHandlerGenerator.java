...
public class FunctionHandlerGenerator {
    ...
    public FunctionDeploymentContext generateHandlerFunction(FunctionDefinitionNode sourceFunc)
        ...
        TypeDescriptorNode originalFunctionTypeDesc =
                GeneratorUtil.getCheckedReturnTypeDescOfOriginalFunction(ctx.getSourceFunction());

        boolean isCheckRequired = GeneratorUtil.isCheckingRequiredForOriginalFunction(ctx.getSourceFunction());
        String returnName = GeneratorUtil.addFunctionCallStatement(originalFunctionTypeDesc, ctx,
                GeneratorUtil.createFunctionInvocationNode(sourceFunc.functionName().text(),
        ...
        if (returnHandler != null) {
            returnHandler.postInvocationProcess(GeneratorUtil.createVariableRef(returnName));
        }
        return ctx;
    }
    ...
    private FunctionDeploymentContext createFuncDeplContext(FunctionDefinitionNode sourceFunc)
        ...
        ctx.setIsolatedFunction(GeneratorUtil.isIsolatedFunction(sourceFunc));
        ctx.setFunction(GeneratorUtil.createHandlerFunction(ctx));
