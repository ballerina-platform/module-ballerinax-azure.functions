...
public class BlobInputParameterHandler extends AbstractParameterHandler {
    ...
    public ExpressionNode invocationProcess() throws AzureFunctionsException {
        ...
        if (GeneratorUtil.isOptionalByteArrayType(this.variableSymbol)) {
            ...
            return GeneratorUtil
                    .createAfFunctionInvocationNode("getOptionalBytesFromInputData", true, paramsArg, stringArg);
        } else if (GeneratorUtil.isOptionalStringType(this.variableSymbol)) {
            PositionalArgumentNode stringArg =
                    NodeFactory.createPositionalArgumentNode(GeneratorUtil.createStringLiteral(this.name));
            return GeneratorUtil
