...
public class BlobTriggerParameterHandler extends AbstractParameterHandler {
    ...
    public ExpressionNode invocationProcess() throws AzureFunctionsException {
        ...
        if (GeneratorUtil.isByteArrayType(this.variableSymbol)) {
            PositionalArgumentNode stringArg =
                    NodeFactory.createPositionalArgumentNode(GeneratorUtil.createStringLiteral(this.name));
            return GeneratorUtil.createAfFunctionInvocationNode("getBytesFromInputData", true, paramsArg, stringArg);
        } else if (GeneratorUtil.isStringType(this.variableSymbol)) {
            PositionalArgumentNode stringArg =
                    NodeFactory.createPositionalArgumentNode(GeneratorUtil.createStringLiteral(this.name));
            return GeneratorUtil.createAfFunctionInvocationNode("getStringConvertedBytesFromInputData", true, paramsArg,
                    stringArg);
        } else {
