 ...
 * under the License.
 */
package org.ballerinax.azurefunctions.generator.validators;
 ...
 * @since 2.0.0
 */
public class AzureFunctionsCodeAnalyzerTask implements AnalysisTask<CompilationAnalysisContext> {
    ...
    public void perform(CompilationAnalysisContext compilationAnalysisContext) {
        ...
        if (currentPackage.project().kind() != ProjectKind.BUILD_PROJECT) {
            DiagnosticInfo diagnosticInfo = new DiagnosticInfo("A000", "azure functions are only allowed in ballerina" +
                    " projects", DiagnosticSeverity.ERROR);
