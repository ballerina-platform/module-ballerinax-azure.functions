...
public class MainFunctionValidator extends NodeVisitor {
    ...
    private boolean isAzureFunctionsImportExist = false;
    ...
    public void visit(ImportDeclarationNode importDeclarationNode) {
        ...
        if (Constants.AZURE_FUNCTIONS_MODULE_NAME.equals(moduleName)) {
            isAzureFunctionsImportExist = true;
        }
    }

    @Override
    public void visit(FunctionDefinitionNode functionDefinitionNode) {
        if (!isAzureFunctionsImportExist) {
            return;
        }
        ...
        if (Constants.MAIN_FUNC_NAME.equals(text)) {
            DiagnosticInfo diagnosticInfo = new DiagnosticInfo("AZ010", "main function is not allowed in " +
                    "azure functions", DiagnosticSeverity.ERROR);
